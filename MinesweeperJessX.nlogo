...
to mark [flags]
    if not empty? flags
       [let x item 0 flags
         ...
         ask patch x y [
             ifelse any? mines-here [
               if not any? markers-here
                 [ sprout-markers 1 [ set color red show-turtle] ]
             ]
             [show (word "BAD FLAG: " x "-" y) set game-over? true ask grass-squares-here [die] set plabel "OH" stop]
        ]
